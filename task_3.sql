...
select a.account_id,round((a.balance + ifnull(sum(case when t.transaction_type = 'withdrawal' then t.amount * -1
when t.transaction_type = 'deposit' then t.amount else 0 end), 0)) / 2, 2) as approx_avg_daily_balance
from Accounts a left join Transactions t on a.account_id = t.account_id and t.transaction_date between '2025-04-01' and '2025-04-30'
